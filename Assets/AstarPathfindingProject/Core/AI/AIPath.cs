...
namespace Pathfinding {
	...
	/// Since this script will register when waking up to the <see cref="Pathfinding.Seeker.pathCallback"/> delegate, this script will be notified every time a new path is calculated by the <see cref="OnPathComplete"/> method being called.
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		/// </summary>
		[Tooltip("How quickly the agent accelerates. Positive values represent an acceleration in world units per second squared. Negative values are interpreted as an inverse time of how long it should take for the agent to reach its max speed. For example if it should take roughly 0.4 seconds for the agent to reach its max speed then this field should be set to -1/0.4 = -2.5. For a negative value the final acceleration will be: -acceleration*maxSpeed. This behaviour exists mostly for compatibility reasons. In the Unity inspector there are two modes: Default and Custom. In the Default mode this field is set to -2.5 which means that it takes about 0.4 seconds for the agent to reach its top speed. In the Custom mode you can set the acceleration to any positive value.")]
		...
		[Tooltip("Distance from the end of the path where the AI will start to slow down.")]
		...
		[Tooltip("Draws detailed gizmos constantly in the scene view instead of only when the agent is selected and settings are being modified")]
		...
		/// </summary>
		[Tooltip("Slow down when not facing the target direction. Incurs at a small performance overhead.")]
